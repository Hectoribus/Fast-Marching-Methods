...
bool containsPoint(heap* h, int x, int y, int type)
{
    for (int i = 0; i < h->size; i++) {
        if ((h->arr[i].x == x) && (h->arr[i].y == y) && (h->arr[i].type == type)) {
...
bool get_neighbors(Point x_min, Point neighbors[4], int *n_num, heap* HEAP_Narrow_Band, int size, Point c_points[2]){
    *n_num = 0;
    int nx, ny;
    ...
    for(int dx = -1; dx <= 1; dx = dx+2){
        ...
        if((0 <= nx) && (nx < GRID_SIZE) && (0 <= ny) && (ny < GRID_SIZE)){
            ...
            (*n_num)++;
            if((type == 0) && (containsPoint(HEAP_Narrow_Band, nx, ny, 1))){
                //   containsPoint(heap* h, nx, ny, 1)
                //printf("CONNECTION_1\n");
                connection = 1;
                c_points[0].x = x; 
                c_points[0].y = y;
            ...
            if((type == 1) && (containsPoint(HEAP_Narrow_Band, nx, ny, 0))){
                //printf("CONNECTION_2\n");
                connection = 1;
                c_points[0].x = nx; 
                c_points[0].y = ny;
...
void FMM(double **Grid_MA, float *V_field_MA, Point *sources, int num_sources, float *Time_MA, Point connection_points[2]) {
    ...
    while ((HEAP_Narrow_band->size != 0) && (connection == 0)) { // MIENTRAS QUE EL ARRAY NARROW NO ESTÉ VACÍO (0 EN TODAS LAS POSICIONES)
        ...
        */
        ...
        connection = get_neighbors(x_min, neighbors, &n_num, HEAP_Narrow_band, size, c_points);
